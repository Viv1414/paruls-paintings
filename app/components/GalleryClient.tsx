...
import { Painting } from '@/lib/getPaintings'
...
export default function GalleryClient({ paintings }: { paintings: Painting[] }) {
  ...
  return (
    <main className="min-h-screen py-16 px-6 max-w-7xl mx-auto">
      ...
      {/* Filters */}
      <div className="rounded-2xl border-2 border-black p-6 mb-10 flex flex-wrap gap-6 items-end" style={{ backgroundColor: '#fef08a' }}>
        <div>
            ...
            <Link href={`/gallery/${p.id}`} key={p.id}>
              <div className="group rounded-2xl overflow-hidden border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all cursor-pointer">
                <div className="bg-gray-100 aspect-square overflow-hidden relative">
                  ...
                  {p.favourite && (
                    <span className="absolute top-3 left-3 text-xs font-bold px-3 py-1 rounded-full border-2 border-black" style={{ backgroundColor: '#f9a8c9' }}>
                      ⭐ Favourite
                    </span>
                  )}
                  {p.sold && (

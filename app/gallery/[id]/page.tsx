import { getPainting } from '@/lib/getPaintings'
  ...
  return (
    <main className="min-h-screen py-16 px-6">
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        <div>
          {painting.favourite && (
            <span className="inline-block text-sm font-bold px-4 py-1 rounded-full border-2 border-black mb-4" style={{ backgroundColor: '#f9a8c9' }}>
              ⭐ Parul's Favourite
            </span>
          )}

          <h1 className="text-4xl font-bold mb-1">{painting.title}</h1>
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            {[
              ...
              <div key={item.label} className="rounded-xl border-2 border-black p-3" style={{ backgroundColor: '#fef08a' }}>
          ...
          <div className="mb-8">
            ...
            <div className="flex gap-2 flex-wrap">
              {painting.colours.map(c => (
                <span key={c} className="px-3 py-1 rounded-full border-2 border-black text-sm font-medium capitalize" style={{ backgroundColor: '#dcfce7' }}>{c}</span>
              ))}
            </div>
          </div>
